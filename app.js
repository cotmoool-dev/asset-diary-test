...
const APP_BUILD = 'v1.28.0'; // sw.js의 CACHE 버전과 항상 맞춰서 올릴 것 — 설정 화면에 그대로 노출해서, 실제 폰에 반영된 버전을 화면 캡처 하나로 바로 확인할 수 있게 함
...
function purposeEmo(p) { return PURPOSE_EMO[p] || '🏷️'; }
  ...
  <details class="more no-print">
    ...
    <section class="card tape t3">
      ${allPurposes().map(p => { const v = T.byPurpose[p] || 0; const w = T.value ? v / T.value * 100 : 0;
        return `<div class="hbar"><span>${E(purposeEmo(p))} ${esc(p)}</span><div class="track"><div class="fill" style="width:${w}%"></div></div><span class="num">${wonShort(v)} · ${pct(w, 0)}</span></div>`; }).join('')}
    </section>
  </details>
  ${gapMini(T)}
  <details class="more no-print">
...
function assetItem(a) {
  ...
  let sub = `${purposeEmo(a.purpose)} ${esc(a.purpose)}`;
...
function assetForm(a, draftOverride) {
  ...
  const html = `
    ...
    <div class="row2">
      ...
      <label class="field"><span>목적</span><select class="input" id="f_purpose">${opts(allPurposes(), a.purpose, purposeEmoMap())}<option value="__new__">➕ 직접 입력…</option></select></label>
      <div id="newPurposeWrap" hidden>
        ...
        <p class="hint">한 번 적어두면 다음부터 이 목록에서 바로 고를 수 있어요.</p>
      </div>
    </div>
...
function goalForm(id) {
  ...
  const html = `
    ...
    <label class="field"><span>집계할 목적 태그</span><select class="input" id="g_purpose">${opts(allPurposes(), g.purpose, purposeEmoMap())}</select></label>
